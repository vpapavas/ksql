...
public class PullQueryResource implements KsqlConfigurable {
  // CHECKSTYLE_RULES.ON: ClassDataAbstractionCoupling

  private static final Logger LOG = LoggerFactory.getLogger(PullQueryResource.class);
  ...
  private Response handleQuery(
    ...
    final PullQueryResponse pullQueryResponse = new PullQueryResponse(
        (TransientQueryMetadata) query,
        disconnectCheckInterval.toMillis(),
        objectMapper);

    LOG.info("Executing query '{}'", statement.getStatementText());
    return Response.ok().entity(pullQueryResponse.getResponse()).build();
  }

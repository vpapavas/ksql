...
@UdfDescription(name = "randomstr", author = KsqlConstants.CONFLUENT_AUTHOR,
...
public class RandomStr {
  ...
  @Udf(description = RandomStr.DESCRIPTION)
  public String randomstr(
      @UdfParameter(value = "min", description = "minimum number of characters in string")
      final int min,
      @UdfParameter(value = "max", description = "maximum number of characters in string")
      final int max
  ) {
    if (min < 1 || max < min) {
      return null;
